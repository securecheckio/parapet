    ...
    S: ApiStateAccess,
{
    Router::new()
        ...
        .route("/api/v1/rules/:rule_id", delete(routes::rules::delete_rule::<S>))
        .route("/api/v1/rules/export", post(routes::rules::export_rules::<S>))
        .route("/api/v1/rules/import", post(routes::rules::import_rules::<S>))
        
        // Authentication (nonce generation)
        .route("/api/v1/auth/nonce", post(routes::auth::generate_nonce::<S>))
        
        // Escalation API
        .route("/api/v1/escalations/:escalation_id", get(routes::escalations::get_escalation::<S>))
        .route("/api/v1/escalations/:escalation_id/approve", post(routes::escalations::approve_escalation::<S>))
        .route("/api/v1/escalations/:escalation_id/deny", post(routes::escalations::deny_escalation::<S>))
        .route("/api/v1/escalations/:escalation_id/status", get(routes::escalations::get_status::<S>))
        .route("/api/v1/escalations/pending", post(routes::escalations::list_pending::<S>))
        
        // WebSocket for real-time notifications
        .route("/ws/escalations", get(routes::websocket::escalation_websocket_handler::<S>))
...
pub mod config {
    ...
    pub fn load_config_from_file(path: &str) -> Result<Config> {
        ...
        let mut config = Config {
            ...
            solana_network: toml_config.solana.network.unwrap_or_else(|| "mainnet-beta".to_string()),
