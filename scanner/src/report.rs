...
use crate::detector::{ThreatAssessment, Severity};
...
pub async fn generate_report(
    ...
    let critical_count = threats.iter().filter(|t| matches!(t.severity, Severity::Critical)).count();
    let high_count = threats.iter().filter(|t| matches!(t.severity, Severity::High)).count();
    let medium_count = threats.iter().filter(|t| matches!(t.severity, Severity::Medium)).count();
    let low_count = threats.iter().filter(|t| matches!(t.severity, Severity::Low)).count();
