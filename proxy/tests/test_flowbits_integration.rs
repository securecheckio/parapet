...
async fn test_ai_agent_velocity_limit() {
    ...
    for i in 0..9 {
        ...
        assert_ne!(decision.action, parapet_core::rules::types::RuleAction::Block, 
                   "Transaction {} should not be blocked", i + 1);
    }
    ...
    let decision = engine.evaluate(&tx).await.unwrap();
    assert_eq!(decision.action, parapet_core::rules::types::RuleAction::Block);
...
async fn test_ai_agent_account_spam() {
    ...
    for i in 0..4 {
        ...
        let decision = engine.evaluate(&tx).await.unwrap();
        assert_ne!(decision.action, parapet_core::rules::types::RuleAction::Block,
                   "Account creation {} should not be blocked", i + 1);
    }
...
async fn test_enterprise_lateral_movement() {
    ...
    assert_ne!(decision1.action, parapet_core::rules::types::RuleAction::Block,
    ...
    assert_ne!(decision2.action, parapet_core::rules::types::RuleAction::Block,
    ...
    assert_eq!(decision3.action, parapet_core::rules::types::RuleAction::Block,
               "Third transfer should be blocked (lateral movement)");
    assert!(decision3.message.contains("Lateral movement") || decision3.message.contains("lateral"));
}
...
async fn test_ai_agent_gradual_exfiltration() {
    ...
    for i in 0..3 {
        ...
        let decision = engine.evaluate(&tx).await.unwrap();
        assert_ne!(decision.action, parapet_core::rules::types::RuleAction::Block,
                   "Transfer {} should not be blocked", i + 1);
    }
    ...
    let decision = engine.evaluate(&tx).await.unwrap();
    assert_eq!(decision.action, parapet_core::rules::types::RuleAction::Block);
    assert!(decision.message.contains("exfiltration") || decision.message.contains("Repeated transfers"));
}
