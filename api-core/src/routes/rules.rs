    ...
    S: ApiStateAccess,
{
    ...
    // Verify signature
    let message = format!("parapet:create_rule:{}:{}", req.signed_rule, req.timestamp);
    ...
    if let Some(expires_at) = req.rule.expires_at {
        ...
        redis
            ...
            .map_err(|e| {
                ...
                (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    "Failed to store rule".to_string(),
                )
            })?;
    } else {
        redis
    ...
    S: ApiStateAccess,
{
    ...
    // Check authorization
    if !is_wallet_authorized(&req.wallet, &state.config().authorized_wallets) {
        return Err((StatusCode::FORBIDDEN, "Wallet not authorized".to_string()));
    }
    ...
    let mut redis = redis_conn.clone();
    let keys: Vec<String> = redis.keys("dynamic_rules:*").await.map_err(|e| {
    ...
    for key in keys {
        let rule_json: String = redis.get(&key).await.map_err(|e| {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                "Failed to retrieve rule".to_string(),
            )
        })?;
    ...
    S: ApiStateAccess,
{
    ...
    redis.del::<_, ()>(&rule_key).await.map_err(|e| {
    ...
    S: ApiStateAccess,
{
    ...
    for rule in req.rules {
        ...
        if let Some(expires_at) = rule.expires_at {
            ...
            redis.set::<_, _, ()>(&rule_key, &rule_json).await.ok();
        }
