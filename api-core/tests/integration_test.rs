...
use parapet_api::{routes, state::{AppState, Config}};
use solana_sdk::{signature::{Keypair, Signer}, transaction::Transaction as SolanaTransaction};
...
async fn test_escalation_lifecycle() -> Result<()> {
    ...
    redis.set_ex::<_, _, ()>(
        &escalation_key,
        serde_json::to_string(&escalation)?,
        300
    ).await?;
...
async fn test_pending_transaction_storage() -> Result<()> {
    ...
    assert!(ttl > 0 && ttl <= 50, "TTL should be between 1 and 50 seconds");
...
async fn test_websocket_event_publish() -> Result<()> {
    ...
    // Publish event
    let subscribers: usize = redis.publish(&channel, serde_json::to_string(&event)?).await?;
...
mod canonical_hash_tests {
    ...
    fn test_canonical_hash_determinism() {
        ...
        assert_eq!(hash1, hash2, "Canonical hashes should be identical for same transaction logic");
    }
    ...
    fn test_canonical_hash_different_instructions() {
        ...
        assert_ne!(hash1, hash2, "Canonical hashes should differ for different transaction logic");
    }
}
