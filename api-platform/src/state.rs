...
use parapet_api_core::{middleware::McpRateLimiter, ApiStateAccess};
...
impl PlatformState {
    pub async fn new(api_config: ApiConfig, platform_config: PlatformConfig) -> Result<Self> {
        ...
        log::info!(
            "  - Payments: {}",
            if platform_config.payments.enabled {
                "enabled"
            } else {
                "disabled"
            }
        ...
        log::info!(
            "  - Push notifications: {}",
            if platform_config.push_notifications.enabled {
                "enabled"
            } else {
                "disabled"
            }
