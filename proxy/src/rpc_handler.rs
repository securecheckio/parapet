...
async fn handle_simulate_transaction(
    ...
    if let (Some(tx), Some(result)) = (&transaction, &mut response.result) {
        log::info!("🔍 Running Sol Shield analysis on simulation");
    ...
    log::info!("📥 Simulation completed with Sol Shield analysis");
    (StatusCode::OK, Json(response))
}

/// Build structured Sol Shield metadata for simulation responses
