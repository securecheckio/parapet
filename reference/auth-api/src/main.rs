...
async fn main() -> Result<()> {
    ...
    log::info!("🚀 Starting SecureCheck Auth API");
    ...
    let port: u16 = std::env::var("AUTH_API_PORT")
        ...
        .parse()
        .expect("AUTH_API_PORT must be valid");
    ...
    //     .run(&db)
    //     .await?;
    log::info!("✅ Migrations complete (skipped)");

    // Initialize session store
    let sessions = SessionStore::new(redis_url.clone());
    ...
    let state = AppState { db, sessions, redis };
    ...
    log::info!("🎯 Auth API listening on {}", addr);
...
async fn get_vapid_public_key() -> Result<Json<serde_json::Value>, StatusCode> {
    match std::env::var("VAPID_PUBLIC_KEY") {
        Ok(key) => Ok(Json(serde_json::json!({ "publicKey": key }))),
        Err(_) => {
            log::error!("VAPID_PUBLIC_KEY not set in environment");
...
async fn get_network_info() -> impl IntoResponse {
    let network = std::env::var("SOLANA_NETWORK")
        .unwrap_or_else(|_| "devnet".to_string());
    Json(NetworkInfo { network })
}
...
async fn create_payment(
    ...
    let payments_enabled = std::env::var("PAYMENTS_ENABLED")
...
async fn get_pricing() -> Json<PricingResponse> {
    let payments_enabled = std::env::var("PAYMENTS_ENABLED")
        .unwrap_or_else(|_| "false".to_string())
        .parse::<bool>()
        .unwrap_or(false);
    ...
    let token_info = TokenInfo {
        name: std::env::var("PAYMENT_TOKEN_NAME").unwrap_or_else(|_| "xLABS".to_string()),
        symbol: std::env::var("PAYMENT_TOKEN_SYMBOL").unwrap_or_else(|_| "xLABS".to_string()),
        mint: std::env::var("PAYMENT_TOKEN_MINT").unwrap_or_else(|_| "7B2tQy8DwYt6aXHzt6UVDuqBB6WmykyZQodLSReQ9Wcz".to_string()),
        logo: std::env::var("PAYMENT_TOKEN_LOGO").unwrap_or_else(|_| "https://raw.githubusercontent.com/solana-labs/token-list/main/assets/mainnet/7B2tQy8DwYt6aXHzt6UVDuqBB6WmykyZQodLSReQ9Wcz/logo.png".to_string()),
        decimals: std::env::var("PAYMENT_TOKEN_DECIMALS")
            .ok()
...
async fn get_active_rules(
    ...
    let rules_path = std::env::var("RULES_PATH")
        .unwrap_or_else(|_| "../../sol-shield/proxy/rules/presets/bot-essentials.json".to_string());
