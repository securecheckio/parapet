...
#[command(about = "Evaluate a Solana transaction signature against sol-shield security rules")]
#[command(long_about = "\
Fetches a transaction from the RPC, decodes it, and runs it through the \
sol-shield rule engine. Prints which rules matched, the action taken \
...
async fn main() -> Result<()> {
    ...
    if args.format == "pretty" {
        ...
        println!("{}", "              Sol-Shield Transaction Checker".bright_blue().bold());
        println!("{}", "═══════════════════════════════════════════════════════════".bright_blue().bold());
        println!();
