use super::analyzer::{AnalyzerRegistry, ConfirmedInnerInstruction, ConfirmedTransactionMetadata, TransactionAnalyzer};
use anyhow::Result;
...
impl TransactionAnalyzer for MetadataAwareAnalyzer {
    ...
    async fn analyze_with_metadata(
        ...
        result.insert("inner_ix_count".to_string(), json!(metadata.inner_instructions.len()));
        Ok(result)
    }
}
...
fn test_get_all_fields() {
    ...
    registry.register(Arc::new(
        MockAnalyzer::new("analyzer1").with_fields(vec!["field_a".to_string(), "field_b".to_string()])
    ));
    registry.register(Arc::new(
        MockAnalyzer::new("analyzer2").with_fields(vec!["field_x".to_string(), "field_y".to_string()])
    ));
    ...
    assert!(all_fields.get("analyzer1").unwrap().contains(&"field_a".to_string()));
    assert!(all_fields.get("analyzer2").unwrap().contains(&"field_x".to_string()));
}
...
async fn test_analyze_selected_single() {
    ...
    let result = registry.analyze_selected(&tx, &vec!["test".to_string()]).await.unwrap();
...
async fn test_analyze_selected_multiple() {
    ...
    let tx = create_test_transaction();
    let result = registry.analyze_selected(
        &tx,
...
async fn test_analyze_with_metadata() {
    ...
    let result = registry.analyze_selected_with_metadata(
        &tx,
        &vec!["metadata_analyzer".to_string()],
        &metadata
    ).await.unwrap();
...
async fn test_analyze_with_logs() {
    ...
    let result = registry.analyze_selected_with_logs(
        &tx,
        &vec!["metadata_analyzer".to_string()],
        &logs
    ).await.unwrap();
...
fn test_get_recommended_delay_ignores_unavailable() {
    let mut registry = AnalyzerRegistry::new();
    registry.register(Arc::new(
        MockAnalyzer::new("available").with_delay(100)
    ));
    registry.register(Arc::new(
        MockAnalyzer::new("unavailable")
            .with_delay(500)
...
async fn test_analyzer_trait_default_analyze_with_metadata() {
    ...
    let result = analyzer.analyze_with_metadata(&tx, &metadata).await.unwrap();
