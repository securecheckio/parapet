...
async fn fetch_rules_from_feed_cached(
    ...
    let mut request = client
        .get(url)
        .header("User-Agent", "sol-shield-proxy/1.0");
...
impl FeedUpdater {
    ...
    pub async fn fetch_all_sources(&self) -> Result<MergedRuleFeed> {
        anyhow::bail!(
            "sol-shield-core was built without `reqwest`; rule feed updates are unavailable"
        )
    }
...
mod tests {
    ...
    async fn test_multi_source_fetch() {
        let config = FeedConfig {
            feed_sources: vec![
                FeedSource {
                    url: "https://raw.githubusercontent.com/securecheckio/sol-shield-rules/main/community-feed.json".to_string(),
