...
```bash
cargo run --bin parapet-api

# Or with custom config path:
cargo run --bin parapet-api -- --config /path/to/config.toml
```
...
- **Library** (`parapet_api`): Reusable router, state management, and config loading
- **Binary** (`parapet-api`): Standalone executable
...
```bash
# Check compilation
cargo check -p parapet-api

# Build release
cargo build --release -p parapet-api

# Run tests
cargo test -p parapet-api
```
...
cargo build --release --bin parapet-api

# Copy binary and config
cp target/release/parapet-api /opt/parapet/
cp config.toml /opt/parapet/

# Run (from directory containing config.toml)
cd /opt/parapet && ./parapet-api
```
