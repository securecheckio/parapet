#!/usr/bin/env python3
"""
Parapet Agent Integration Example (Python)

This demonstrates how an AI agent should interact with Parapet RPC proxy.
...
class SolShieldAnalysis:
    """Parapet security analysis result"""
    version: str
    risk_score: int
    structural_risk: int
    simulation_risk: int
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'SolShieldAnalysis':
        """Parse Parapet metadata from simulation response"""
        warnings = [
            SolShieldWarning(
                severity=w['severity'],
                message=w['message'],
...
class SolShieldAgent:
    """AI Agent with Parapet integration"""
    ...
    def health_check(self) -> bool:
        """Check if Parapet is accessible"""
        try:
    ...
    def simulate_transaction(self, tx_base58: str) -> Optional[SolShieldAnalysis]:
        """
        Simulate transaction and return Parapet analysis
        ...
        # Extract Parapet metadata
        sol_shield = result.get('solShield')
        if not sol_shield:
            print("⚠️  No Parapet metadata found - RPC may not be protected")
            return None
    ...
    def send_transaction_safely(self, tx_base58: str) -> Optional[str]:
        """
        Send transaction with Parapet protection
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test Parapet agent integration")
    parser.add_argument("--rpc", default="http://localhost:8899", help="Parapet RPC URL")
    ...
    if agent.health_check():
        print("✅ Parapet is accessible")
    else:
        print("❌ Parapet is not accessible")
        sys.exit(1)
    print("")
