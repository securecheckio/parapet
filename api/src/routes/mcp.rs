...
fn handle_list_resources() -> Result<Value, McpError> {
    Ok(json!({
        "resources": [
            {
                "uri": "sol-shield://guide",
                "name": "Sol-Shield Guide",
                "description": "Complete guide to using Sol-Shield for wallet and program security analysis",
                "mimeType": "text/markdown"
            },
            {
                "uri": "sol-shield://risk-scoring",
                "name": "Risk Scoring System",
                "description": "Explanation of how Sol-Shield calculates risk scores and threat levels",
                "mimeType": "text/markdown"
            },
            {
                "uri": "sol-shield://examples",
                "name": "Usage Examples",
...
fn handle_resource_read(params: Value) -> Result<Value, McpError> {
    ...
    let content = match uri {
        "sol-shield://guide" => include_str!("../../../mcp/resources/guide.md"),
        "sol-shield://risk-scoring" => include_str!("../../../mcp/resources/risk-scoring.md"),
        "sol-shield://examples" => include_str!("../../../mcp/resources/examples.md"),
...
async fn handle_tool_call(params: Value, state: &AppState, api_key: &str) -> Result<Value, McpError> {
...
async fn scan_wallet_tool(args: Value, state: &AppState, api_key: &str) -> Result<Value, McpError> {
...
async fn analyze_program_tool(args: Value, state: &AppState) -> Result<Value, McpError> {
    let program_id = args
        .get("program_id")
        .and_then(|v| v.as_str())
        .ok_or_else(|| McpError {
...
async fn generate_revoke_transaction_tool(args: Value, state: &AppState) -> Result<Value, McpError> {
    let wallet = args
        .get("wallet_address")
        .and_then(|v| v.as_str())
        .ok_or_else(|| McpError {
...
async fn generate_batch_revoke_tool(args: Value, state: &AppState) -> Result<Value, McpError> {
    let wallet = args
        .get("wallet_address")
        .and_then(|v| v.as_str())
        .ok_or_else(|| McpError {
...
async fn build_emergency_lockdown_tool(args: Value, state: &AppState, _api_key: &str) -> Result<Value, McpError> {
