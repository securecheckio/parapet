...
impl AnalyzersConfig {
    ...
    pub fn should_register(&self, analyzer_name: &str) -> bool {
        if !self.is_enabled(analyzer_name) {
            log::info!("⏭️  Skipping analyzer '{}' (disabled in config)", analyzer_name);
            return false;
        }

        if !self.requirements_met(analyzer_name) {
            log::info!("⏭️  Skipping analyzer '{}' (requirements not met)", analyzer_name);
            return false;
        }
...
"#;
        ...
        assert!(config.is_enabled("test_analyzer"));
        assert_eq!(config.analyzers.get("test_analyzer").unwrap().description, "Test analyzer");
    }
...
"#;
        ...
        assert_eq!(analyzer_config.requires_api_key, Some("MY_API_KEY".to_string()));
    }
...
"#;
        ...
        let config: AnalyzersConfig = toml::from_str(toml_str).unwrap();
        let analyzer_config = config.analyzers.get("test_analyzer").unwrap();
        assert_eq!(analyzer_config.requires_feature, Some("my_feature".to_string()));
    }
