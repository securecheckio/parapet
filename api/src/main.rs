...
async fn main() -> Result<()> {
    ...
    log::info!("🚀 Starting Parapet API Service");
    ...
    let app = Router::new()
        ...
        // Escalation API
        .route("/api/v1/escalations/:escalation_id", get(routes::escalations::get_escalation))
        .route("/api/v1/escalations/:escalation_id/approve", post(routes::escalations::approve_escalation))
        .route("/api/v1/escalations/:escalation_id/deny", post(routes::escalations::deny_escalation))
        .route("/api/v1/escalations/:escalation_id/status", get(routes::escalations::get_status))
        .route("/api/v1/escalations/pending", post(routes::escalations::list_pending))
        
        // WebSocket for real-time notifications
        .route("/ws/escalations", get(routes::websocket::escalation_websocket_handler))
