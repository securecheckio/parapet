#!/usr/bin/env ts-node
/**
 * Parapet Agent Integration Example (TypeScript)
 * 
...
// Type definitions for Parapet metadata
...
class ParapetAgent {
  ...
  /**
   * Health check - verify Parapet is accessible
   */
  ...
  /**
   * Simulate transaction and get Parapet analysis
   */
  async simulateTransactionSafe(tx: Transaction): Promise<{
    ...
    try {
      ...
      if (!solShield) {
        console.warn('⚠️  No Parapet metadata - RPC may not be protected');
  ...
  async sendTransactionSafely(
    ...
    try {
      ...
      // Handle Parapet specific errors
      if (error.code === -32004) {
        console.error('🚫 BLOCKED by Parapet:', error.message);
...
async function demo() {
  ...
  console.log('🤖 Parapet Agent Integration Demo');
  console.log('=====================================\n');
  ...
  if (isHealthy) {
    console.log('✅ Parapet is running and accessible\n');
  } else {
    console.error('❌ Parapet is not accessible\n');
    process.exit(1);
  }
